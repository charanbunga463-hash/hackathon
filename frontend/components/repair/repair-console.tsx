...
import { Badge, Button, Card, CardHeader, ErrorNote, LinkButton } from "@/components/ui/primitives";
 ...
 *
 * Every stage is always shown, including the ones still ahead — the point is
 * that the pipeline is fixed and nothing gets skipped, which you cannot see
 * from a progress bar that only draws what has already happened.
 */
              ...
              <span
                className={cn(
                  "flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-2xs font-semibold transition-colors",
                    ...
                    : active
                      ? "border-brand-line bg-brand-soft text-brand-ink shadow-subtle"
 ...
 * The approval gate.
 *
 * This is the only place a patch can be written to a workspace, and it will not
 * do so until someone clicks. The diff is shown in full above the buttons on
 * purpose — approving without seeing the change should not be possible.
 */
export function ApprovalGate({
  ...
  return (
    <Card className="border-warn-line ring-1 ring-warn/20">
      <CardHeader
      ...
      <div className="space-y-4 p-4">
        <div>
          <p className="text-sm font-semibold text-ink">{patch.title}</p>
        ...
        <div className="flex flex-wrap gap-2">
          <Button variant="success" loading={busy === "approve"} onClick={() => decide(true)}>
            <Check className="h-4 w-4" /> Apply patch
          </Button>
          <Button variant="danger" loading={busy === "reject"} onClick={() => decide(false)}>
            <X className="h-4 w-4" /> Reject
          </Button>
        </div>

        <p className="rounded-lg border border-line bg-sunken px-3 py-2 text-2xs leading-relaxed text-muted">
          Approving applies the patch to your workspace, then runs the targeted tests and the full
          suite. A snapshot is taken first, so the change can always be rolled back.
        </p>
      </div>
    </Card>
  );
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <Badge tone={session.reasoning_engine === "openai" ? "brand" : "warn"}>
      ...
      {session.baseline ? (
        <Card>
          <CardHeader
            title="Baseline"
            subtitle="The state of the project before any change"
          />
          ...
          <div className="space-y-6 p-4">
            {attempt.investigation ? (
              <details className="overflow-hidden rounded-xl border border-line">
                <summary className="cursor-pointer bg-elevated/60 px-3.5 py-2.5 text-xs font-semibold text-muted transition-colors hover:text-ink">
                  Investigation — {attempt.investigation.steps.length} step(s),{" "}
                  {attempt.investigation.files_read.length} file(s) read
                </summary>
                <ol className="space-y-1.5 border-t border-line p-3.5">
                  {attempt.investigation.steps.map((step) => (
                    <li key={step.index} className="flex items-start gap-2 text-2xs">
                      <span className="mono w-5 shrink-0 text-right text-faint">{step.index}</span>
                      <span className="mono shrink-0 font-semibold text-brand-ink">
                        {step.tool}
                      </span>
                      <span
                        className={cn(
                          "min-w-0 truncate",
                          step.ok ? "text-muted" : "text-danger-ink",
                        )}
                      >
                        {step.result_summary}
                      </span>
                ...
                {attempt.investigation.notes.length ? (
                  <p className="border-t border-line bg-sunken px-3.5 py-2 text-2xs text-muted">
            ...
            {attempt.diagnosis ? (
              <DiagnosisPanel diagnosis={attempt.diagnosis} failure={session.target_failure} />
            ) : null}

            {attempt.patch ? (
              <ClaimSection
                kind={attempt.verified ? "verified" : "proposed_fix"}
                title={attempt.patch.title}
              >
            ...
            {attempt.validation && !attempt.validation.valid ? (
              <div className="rounded-xl border border-danger-line bg-danger-soft p-3.5">
                <p className="flex items-center gap-1.5 text-xs font-bold text-danger-ink">
                  <XCircle className="h-3.5 w-3.5" aria-hidden />
                  Patch rejected before it was applied
                </p>
                <ul className="mt-2 space-y-1 text-2xs text-danger-ink/90">
                  {attempt.validation.issues.map((issue, index) => (
                    <li key={index}>
                      <span className="mono font-semibold">[{issue.code}]</span> {issue.message}
                    </li>
                  ))}
                </ul>
              </div>
      ...
      <div className="flex flex-wrap items-center justify-end gap-2">
        <LinkButton
          ...
          download
          variant={applied && !running ? "primary" : "secondary"}
          size="sm"
        >
